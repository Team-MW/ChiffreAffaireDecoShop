  ...
  <div class="max-w-xl mx-auto w-full pb-10">
    ...
    <Card class="border-slate-200/60 shadow-md overflow-hidden relative">
      ...
      >
        ...
        <h3 class="text-xl font-bold text-slate-900">CA ajouté !</h3>
        <p class="text-slate-500 mt-2 text-sm">Redirection en cours...</p>
      </div>

      <CardHeader class="border-b border-red-100 bg-red-50/50 pb-6">
        <CardTitle class="text-2xl text-red-700">Nouveau CA</CardTitle>
        <CardDescription>Saisissez le Chiffre d'Affaires.</CardDescription>
      </CardHeader>
      
      <form @submit.prevent="handleSubmit">
        <CardContent class="space-y-5 pt-6">
          <div class="space-y-2.5">
            <Label for="date" class="text-slate-600">Date du CA</Label>
            <Input 
              id="date" 
              type="date" 
              v-model="formData.date" 
              class="h-11 shadow-sm focus-visible:ring-red-500 transition-all"
              required 
            />
          </div>
          ...
          <div class="space-y-2.5">
            ...
            <div class="relative">
              ...
              <Input 
                ...
                placeholder="0.00" 
                class="pl-8 h-12 text-lg font-semibold shadow-sm focus-visible:ring-red-500 transition-all"
                required 
              />
            </div>
          </div>
        ...
        <CardFooter class="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 pb-6 bg-slate-50/30 border-t border-slate-50">
          ...
          <Button 
            type="submit" 
            class="w-full sm:w-auto h-11 px-8 bg-red-600 hover:bg-red-700 text-white shadow-md hover:shadow-lg transition-all"
            :disabled="isSubmitting"
          >
            <Loader2 v-if="isSubmitting" class="w-4 h-4 mr-2 animate-spin" />
            <span v-if="isSubmitting">Enregistrement...</span>
            <span v-else>Valider le CA</span>
          </Button>
        </CardFooter>
      </form>
    </Card>
